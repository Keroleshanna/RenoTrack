...
| OQ-3 | ما مزود إرسال البريد الإلكتروني؟ (SMTP شركة قائم، أو مزود مثل SendGrid/Postmark) | Phase 9 (Email) | **تم تقسيمه وحُسم جزئيًا في 2026-08-09 قبل بدء Phase 9** (`ARCHITECTURE_DECISIONS.md` D68). **OQ-3a (آلية النقل) — محسوم: SMTP عبر MailKit** خلف `IEmailSender` كما هي. **OQ-3b (صندوق البريد الفعلي وهوية المُرسِل) — مؤجَّل إلى وقت النشر عمدًا، ولا يحجب أي عمل**: المضيف والمنفذ وبيانات الاعتماد وعنوان المُرسِل واسم العرض ومستقبِلو إشعارات الـ Admin كلها إعدادات لكل عملية نشر، بلا أي قيمة مضمَّنة في الكود وبلا قيمة افتراضية |
...
**✅ OQ-3 لم يعد حاجزًا: OQ-3a محسوم (SMTP + MailKit، D68)، وOQ-3b مؤجَّل إلى وقت النشر عمدًا. لا يوجد صندوق بريد حقيقي بعد، وهذا لا يمنع بناء المرحلة لأن أي قيمة خاصة بالشركة إعدادٌ بلا قيمة افتراضية، ولأن البيئات غير الإنتاجية ترفض الإرسال افتراضيًا.**
**نطاق المرحلة اتّسع بقرار صريح من الـ Product Owner (2026-08-09): Phase 9 هي دورة تسليم البريد الكاملة، وليست تكامل SMTP فقط.**
**ما الذي سنبنيه:**
- تنفيذ `IEmailSender` عبر MailKit/SMTP في Infrastructure، مع `EmailOptions` يفشل عند الإقلاع مسمّيًا المفتاح الناقص.
- **ستة** قوالب بالألمانية (FR-9.3): Angebot token link، Invoice token link، Lead جديد من الموقع، Angebot submitted for review، قرار العميل، **وطلب التعديلات إلى الـ Inspector** (Sequence Diagram §5 — كان مفقودًا من قائمة الخمسة في Architecture §10 وصُحِّح).
- **سياسة الفشل (D69):** العملية التجارية المُلتزَمة تبقى ناجحة إذا فشل البريد؛ الـ API لا يحوّلها إلى 500 أبدًا — وهذا يشمل المسارَين المجهولَين (نموذج التواصل، وقرار العميل بالتوكن).
- **سجل إشعارات مملوك لـ Infrastructure (D69) + Migration رقم 9**: يجعل الفشل مرئيًا للـ Admin. يُكتب السطر *بعد* الالتزام لا داخله، وفجوة الانهيار الناتجة مقبولة وموثَّقة.
- **رؤية الـ Admin للإشعارات الفاشلة/المعلّقة + إعادة محاولة يدوية (D70)**: يدوية فقط، متزامنة، بلا مؤقّت وبلا حد أقصى للمحاولات، وتعيد إرسال **الإشعار فقط** — لا تعيد تنفيذ العملية التجارية أبدًا. `PermissionMatrix.md` §9 (Admin `F` / Inspector `—`).
- **مستقبِلو الإشعارات قائمة إعدادات على مستوى الشركة (D71)**، مستقلة عمدًا عن دور الـ Admin في Identity.
- **لا** message broker، **لا** Outbox، **لا** BackgroundService/IHostedService، **لا** إعادة محاولة تلقائية — نصّ Architecture §10 السابق الذي طلب "queued/retried" حُدِّث ليطابق هذا القرار.
**ما الذي سيكون Working بعد انتهائها:** كل الإشعارات المذكورة في FR-9.1/9.2 تصل فعليًا كبريد إلكتروني حقيقي؛ وإذا فشل الإرسال تبقى العملية التجارية ناجحة، ويرى الـ Admin الفشل ويعيد المحاولة بنفسه.
**Git Branch:** `feature/phase-9-email-integration`
**Pull Request:** `Phase 9: Real email delivery — SMTP/MailKit, German templates, notification status & retry`
**المتطلبات السابقة:** Phase 8. (قرار OQ-3a مُسجَّل؛ لم يعد شرطًا معلّقًا.)
...
**ما الذي سنبنيه:**
- A1 (Home)، A2 (نموذج تواصل → `POST /api/v1/leads` العام)، صفحات Impressum/Datenschutzerklärung (FR-1.4، قانوني إلزامي). **صُحِّح في Phase 11 Slice 7:** آليةُ الصفحتين القانونيتين وهويةُ الشركة بُنيتا مبكرًا في **Phase 11 Slice 7** (`D100`)، تمامًا كما أخذت Phase 11 نصفَ الروابط المؤقتة من هذه المرحلة. ما بقي هنا هو A1 وA2 فقط. **والمحتوى القانوني الحقيقي ليس ملكًا لأي مرحلة** — يُورَّد من الشركة (SRS §5)، وحتى ذلك الحين تُرجع الصفحتان 404 ولا يُخترَع لهما نص.

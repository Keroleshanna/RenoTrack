...
9. **`ICatalogItemQueries` — next up.** First query implementation (not a repository) — `SearchAsync()` projects directly to `CatalogItemDto`, bypassing aggregate hydration, and always excludes retired items (BR-12/D37). Likely needs a brief real review of the projection approach, since this is a genuinely different shape from every repository slice so far.
6. `IAngebotRepository`
...
**Immediate next step:** Slice 9 (`ICatalogItemQueries`). First query implementation in Infrastructure — `SearchAsync()` returns `IReadOnlyList<CatalogItemDto>` directly (a projection, not a hydrated aggregate), always excluding `IsRetired` items, no parameters (BR-12/D37). This is a genuinely new shape relative to every repository slice so far, so it likely warrants a real (if brief) review of the projection/mapping approach before implementing — not the full original design-review template, but not a zero-review slice either.
...
3. Run `dotnet build RenoTrack.slnx` and `dotnet test RenoTrack.slnx` yourself and confirm the counts in `PROJECT_STATE.md` §3 still hold (343 as of Slice 8: 153 Domain + 144 Application + 46 Infrastructure). If they don't, something changed since this handoff was written — investigate before proceeding, don't just trust the stale numbers.
4. Continue with the next slice in `PHASE3_PROGRESS.md`'s order (§1b above) — Slice 9, `ICatalogItemQueries`, unless a later slice's commit has already landed since this was written. Every slice: implementation → `RenoTrack.Infrastructure.Tests` integration tests → documentation updates → commit, in that order, without exception. From Slice 7 onward, a detailed design review is reserved only for a genuinely new architectural question — otherwise state in a few sentences that no new decision is needed and proceed directly (per the user's standing instruction).

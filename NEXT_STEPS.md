...
7. **`IAngebotReviewCommentRepository` — next up.**
6. `IAngebotRepository`
...
**Immediate next step:** Slice 7 (`IAngebotReviewCommentRepository`). This interface has **`AddAsync` only** — no `GetByIdAsync`, since no current command reads a comment back by id (`PermissionMatrix.md` §4's "view review comment history" isn't built as a command yet). The focused review should confirm this single-method, asymmetric contract is still correct rather than assuming a matching `GetByIdAsync` is owed. Every slice follows the same vertically-complete process as Phase 2: design review → implementation → `RenoTrack.Infrastructure.Tests` integration tests → documentation updates → commit. No partially-finished infrastructure, no "configure this later."

...
8. **`ICatalogItemRepository` — next up.**
6. `IAngebotRepository`
...
**Immediate next step:** Slice 8 (`ICatalogItemRepository`). Same `AddAsync`/`GetByIdAsync` shape as `LeadRepository` — independent aggregate, no children, no navigation. Per the now-optimized Phase 3 review process (design reviews reserved for genuinely new architectural questions only), expect a brief confirmation rather than a full review unless `CatalogItem` surfaces something new. Every slice follows the same vertically-complete process as Phase 2: implementation → `RenoTrack.Infrastructure.Tests` integration tests → documentation updates → commit.

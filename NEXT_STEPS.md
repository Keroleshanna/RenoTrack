...
14. **`AddInfrastructure()` DI extension + `Program.cs` wiring — next up.** First slice touching `RenoTrack.Api`. Registers every repository/query/service built in Slices 4–13, matching `DbContext`'s Scoped lifetime (D48).
6. `IAngebotRepository`
...
**Immediate next step:** Slice 14 (`AddInfrastructure()` DI extension + `Program.cs` wiring). Registers `RenoTrackDbContext`, every repository/query built in Slices 4–9, `UnitOfWork`, `AuditService`, `NumberGeneratorService`, `PlaceholderFileStorage`, `LoggingNoOpEmailSender` — all Scoped to match `DbContext`'s own lifetime (D48's finding that this pairing is what makes "repository adds, `UnitOfWork.SaveChangesAsync()` commits" work at all). First slice to touch `RenoTrack.Api`'s `Program.cs` — expect a real, if focused, review of lifetime/configuration-source choices, not a zero-review slice.
...
3. Run `dotnet build RenoTrack.slnx` and `dotnet test RenoTrack.slnx` yourself and confirm the counts in `PROJECT_STATE.md` §3 still hold (358 as of Slice 13: 153 Domain + 144 Application + 61 Infrastructure). If they don't, something changed since this handoff was written — investigate before proceeding, don't just trust the stale numbers.
4. Continue with the next slice in `PHASE3_PROGRESS.md`'s order (§1b above) — Slice 14, `AddInfrastructure()` + `Program.cs` wiring, unless a later slice's commit has already landed since this was written. Every slice: design review (full or abbreviated, per below) → implementation → `RenoTrack.Infrastructure.Tests` integration tests → documentation updates → commit, in that order, without exception. From Slice 7 onward, a detailed design review is reserved only for a genuinely new architectural question — otherwise state in a few sentences that no new decision is needed and proceed directly (per the user's standing instruction). Slice 14 is the first slice touching `RenoTrack.Api` — expect a focused review of DI lifetimes and configuration handling.

...
## 2. `AddAngebotItemCommand` — ✅ Complete (Slice 15)

CatalogItem's Application layer was already complete (§1 above). `AddAngebotItemCommand` was implemented **with both paths from the start** — the entire reason it was postponed (`ARCHITECTURE_DECISIONS.md` D30). See `PHASE2_PROGRESS.md` Slice 15 for the full record. The immediate next task is §3 below: `SaveAngebotItemAsCatalogItemCommand`.

**Confirmed shape, per `Sequence Diagram.md` §4 (lines 188–217, the authoritative wire contract) and BR-8:**
- Command carries an optional `CatalogItemId`. **`Quantity`, `UnitPrice`, and `VatRate` are always caller-supplied, for both paths** — `Sequence Diagram.md` line 210's Catalog-path request body is explicitly `{ catalogItemId, qty, unitPrice, vatRate }`, confirming `UnitPrice` is never silently substituted from `CatalogItem.SuggestedUnitPrice` (SRS FR-4.9: "leaving only quantity **and price** to confirm/adjust"). Only `Description`/`Specification`/`Unit` differ by path: when `CatalogItemId` is supplied, the handler copies `Title`/`DefaultSpecification`/`DefaultUnit` from the loaded `CatalogItem` (BR-8's copy-on-create semantics; `AngebotItem` itself has no knowledge of `CatalogItem` beyond the passive traceability `CatalogItemId` field); when not supplied, the caller provides `Description`/`Specification`/`UnitCode` directly for a fully custom item. (This corrects an earlier, imprecise version of this paragraph that implied `UnitPrice` was also copied — it never is.)
- New DTOs: `ItemDto` and `AngebotSummaryDto` (both named explicitly in Sequence Diagram §4 — "APP-->>API: ItemDto + updated AngebotSummaryDto"), plus a small composite result type wrapping both (`AddAngebotItemResult`) rather than a raw tuple. `AngebotSummaryDto` is a lighter-weight response (Id, AngebotNumber, Status, NetTotal, GrossTotal) distinct from the full `AngebotDto`, used specifically so a client adding items one at a time doesn't need the full DTO shape re-serialized on every add.
- Ownership: `IOwnershipValidator.EnsureAngebotOwnership` (already exists, same as `AddAngebotSectionCommand`).
- **Retired `CatalogItem`s — resolved as BR-14 (`BusinessRules.md`, `ARCHITECTURE_DECISIONS.md` D38): allowed, not rejected.** Retirement only affects discovery (`SearchCatalogItemsQuery`); it never invalidates a `CatalogItemId` as a direct reference. Verified the full documentation set was genuinely silent on this before recording it as a new rule, rather than inferring an answer either way.
- **"Save as Catalog item"** (`SaveAngebotItemAsCatalogItemCommand`) — confirmed deferred to its own follow-up slice after `AddAngebotItemCommand`, not bundled into it (it creates a `CatalogItem`, doesn't touch `Angebot` at all — a genuinely separate command). This is now the immediate next task — see §3.

## 3. Immediate Next Task: `SaveAngebotItemAsCatalogItemCommand`

SRS FR-4.10, Sequence Diagram §4 lines 218–226. Reads an existing `AngebotItem`'s fields (`Title`/`DefaultSpecification`/`DefaultUnit`/`SuggestedUnitPrice` — note: the diagram's `INSERT INTO CatalogItems` step names the target fields as `Title, DefaultSpecification, DefaultUnit, SuggestedUnitPrice, CreatedFromAngebotItemId`, so `AngebotItem.Description`→`CatalogItem.Title`, `AngebotItem.UnitPrice`→`CatalogItem.SuggestedUnitPrice`) and constructs a new `CatalogItem` via `CatalogItem.Create(..., createdFromAngebotItemId: angebotItem.Id)`. Reuses the existing `CatalogItemDto`/`ICatalogItemRepository.AddAsync`. PermissionMatrix.md §6: "Save as Catalog item" is Inspector-**F** (any Inspector, not scoped to the Angebot's owner) — different authorization model from `AddAngebotItemCommand`'s ownership-scoped one, worth confirming explicitly during that slice's design review rather than assuming `IOwnershipValidator` carries over. Once this is done, Phase 2's originally-scoped Lead/Inspection/Angebot/CatalogItem Application-layer work is complete. At that point:

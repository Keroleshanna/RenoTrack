...
**Immediate next step:** Slice 6 (`IAngebotRepository`). `Angebot` has a two-level child tree (`Sections` → `Items`) — `GetByIdAsync` will need `.Include(a => a.Sections).ThenInclude(s => s.Items)`. `IAngebotRepository` also has a third method beyond `AddAsync`/`GetByIdAsync` — `HasActiveAngebotForLeadAsync(int leadId)` (StateMachine §2.4's "one active Angebot per Lead" check) — whose query shape (a `Status`-filtered existence check, not a full aggregate load) needs its own focused review. Every slice follows the same vertically-complete process as Phase 2: design review → implementation → `RenoTrack.Infrastructure.Tests` integration tests → documentation updates → commit. No partially-finished infrastructure, no "configure this later."
...
3. Run `dotnet build RenoTrack.slnx` and `dotnet test RenoTrack.slnx` yourself and confirm the counts in `PROJECT_STATE.md` §3 still hold (326 as of Slice 5: 153 Domain + 144 Application + 29 Infrastructure). If they don't, something changed since this handoff was written — investigate before proceeding, don't just trust the stale numbers.
4. Continue with the next slice in `PHASE3_PROGRESS.md`'s order (§1b above) — Slice 6, `IAngebotRepository`, unless a later slice's commit has already landed since this was written. Every slice: design review → implementation → `RenoTrack.Infrastructure.Tests` integration tests → documentation updates → commit, in that order, without exception. From Slice 4 onward, design reviews are scoped to only what differs from the previously-approved repository, per the user's standing instruction — not the full checklist each time.

...
3. **`RetireCatalogItemCommand` — next up.** Simplest of the three; `CatalogItem.Retire()` takes no parameters and is idempotent (Phase 1b decision — retiring an already-retired item is a no-op, not an error). Expect a trivial handler.

...
| NotificationDeliveries *(designed in the Phase 9 review; **not yet built**)* | Id | (polymorphic: EntityType + EntityId, no DB-level FK — same structural impossibility as `TokenLinks`) | — | **Infrastructure-owned operational record, not business data** (`ARCHITECTURE_DECISIONS.md` D69) — same category as `AuditLogs`, `NumberSequences` and `RefreshTokens`, and like `RefreshTokens` it is deliberately absent from §1's diagram, which models the business domain. Tracks email delivery state so a failed notification is visible to an Admin and retryable, because a committed business operation stays successful when its email fails. Lifecycle `Pending → Sending → Sent`, or `… → Failed → (manual retry) → Sending → Sent`. **The row is written *after* the business commit, not inside it** — the resulting crash window is an accepted, documented trade-off, not an oversight. **Stores no rendered body, no credential, and no copy of business data**: all six notifications are reconstructible from persisted aggregates (Angebot/Invoice/Lead/Customer/TokenLink/AngebotReviewComment), so the row carries identity and delivery state only. Exact columns are derived from this document's conventions and reviewed before implementation; the questions the row must answer are fixed — which notification, which business object, which recipient, status, created-at, last-attempted-at, sent-at, last error, attempt count. **No retention policy** — rows remain until a real requirement exists (contrast `RefreshTokens`) |
...
| NotificationDeliveries *(Phase 9, not yet built)* | Status | The Admin's "failed/pending notifications" read is a status filter, and is the only list query on this table (`PermissionMatrix.md` §9) |
| NotificationDeliveries *(Phase 9, not yet built)* | EntityType, EntityId | Answers "what happened to the notifications for this Angebot/Invoice/Lead" — the same pair, for the same reason, as `AuditLogs` |

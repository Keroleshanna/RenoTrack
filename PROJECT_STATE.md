...
**Phase 4 — API layer**, per `PROJECT_ROADMAP.md`. **In progress** (Slices 1–5 of 11 done). Phases 0–3 are all complete and merged to `main`.
...
As of the last verified run in this conversation, on `feature/phase-4-api-auth-leads-inspections` after Phase 4 Slice 5:
- `dotnet build RenoTrack.slnx` → **0 Warnings, 0 Errors**.
- `dotnet test RenoTrack.slnx` → **440 tests passing, 0 failing.**
- `dotnet ef migrations has-pending-model-changes` → no pending changes (**five** migrations now: `InitialCreate`, `AddAuditLog`, `AddNumberSequence`, `AddIdentity`, `AddRefreshTokens`).
  ...
  - `RenoTrack.Api.Tests`: **69 tests** (Phase 4 — real `WebApplicationFactory<Program>` against real LocalDB, schema via `MigrateAsync`, D58; Slice 1 foundation + Slice 2 ProblemDetails mapping (D59) + Slice 3 DI composition, the last of which discovers handlers/validators by reflection so an unregistered one fails CI).
...
**Phase 4 is in progress — Slices 1–4 of 11 are done.** Resolved along the way: HTTP status mapping for Domain's own exceptions (D59), Application-layer DI composition, and the whole authentication model (D60 — JWT access tokens, persisted rotating refresh tokens stored only as SHA-256 hashes, chain revocation on reuse, and FR-10.3 lockout). The immediate next step is **Slice 5: Lead creation (public)** — the first business endpoint, and the first to prove routing + DI + middleware working together on real Application-layer work.

**Known gap carried forward:** no user account exists in production and nothing creates one (`IdentityRoleSeeder` seeds roles only), so the login endpoint is currently unusable outside tests. This depends on SRS OQ-1 (does Admin manage Inspector accounts?), which is still open — deliberately not answered inside Slice 4. See `PHASE4_PROGRESS.md` for the full slice list and per-slice records. The remainder of this section is the original Phase 4 framing, kept for context.

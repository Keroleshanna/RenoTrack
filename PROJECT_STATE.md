...
> **2026-09-04 — Phase 11 (Customer-Facing Workflow) is in progress on `claude/customer-workflow-phase-rvh4rj`, off `main` at `a5124ca` (the Phase 10 merge, PR #17).** `PHASE11_PROGRESS.md` is the per-slice record and carries the eight design decisions (Q1–Q8) approved before any code was written. **Slice 1 is implemented and awaiting CI verification** — it closes a real correctness defect (D96: two simultaneous customer decisions on the same token link both committed). **The figures in §3 below are Phase 10's and are now stale by one migration (ten → eleven) and ten tests; they will be re-verified at the completion gate.** Everything else in this file remains accurate.
...
**Phase 10 — Dashboard (Angular) — ✅ complete on `feature/phase-10-dashboard` (four commits, off `main` at `6cd8856`), accepted by the Product Owner after a manual end-to-end pass, and being published now.** See `PHASE10_PROGRESS.md`. It delivers the roadmap's Phase 10 **and** Phases 11–12 (Angebot builder, Catalog picker, Angebot review) **and** the Project/Invoice screens, because a Dashboard that stopped at the Lead pipeline could not reach the commercial workflow the system exists for. Every commercial workflow is real and integrated: Lead → Inspection → Angebot (build, submit, review, approve, send) → customer decision → Project → Invoice (create, send, mark paid, void) → Project completion with FR-8.6's override.

**The operational-completion session closed the last gaps between the documented permissions and the running product.** Every endpoint a Dashboard role is permitted to call now has a real workflow behind it — there are no API-client methods without callers. Two workspaces were added (**Katalog**, Wireframe F1; **Benachrichtigungen**, the §9 operational view), and Lead creation, Lead contact correction, Lead and Inspection reassignment, Project hold/resume and Angebot duplication all became usable.

**Phase 10 invented no business behaviour, but it did add five backend enablers** — each one a documented `PermissionMatrix.md` permission that had no route. Every one is a full vertical slice with tests and authorization: manual Lead creation (**D86**), Lead contact correction (**D87**/**D88**), Lead inspector assignment, Inspection reassignment (**D89**), and Project hold/resume (**D90**). **Three of the five needed no new Domain code at all** — `Lead.AssignInspector`, `Project.PutOnHold()` and `Project.Resume()` already existed with correct guards and were simply unreachable. **No new column and no migration — the count stays at nine.** Authorization remains enforced by the API; the Dashboard's guards and capability modules are presentation (**CLAUDE.md §23**) and were verified against real 403s in both role directions.

**One document conflict was resolved rather than absorbed:** `Sequence Diagram.md` §2 depicted manual Lead entry on the anonymous contact form's own route. It could not be implemented as drawn, and the diagram plus `Architecture.md` §5.2 were corrected in the same change (**D86**).

**Three QA rounds followed, and every defect they found was found by operating the product rather than by a test.** Round 1 (`ce91314`) corrected the Quote state machine and the Catalog relationship; round 2 (`5dea592`) added multi-photo upload and camera capture, reusable contact actions, the Leads appointment column and the reopen workflow; round 3 (`090aef0`) was a full browser pass that found two further defects. `PHASE10_PROGRESS.md` §1.4 is the itemised record. **Phase 10 therefore carries one schema change after all** — migration #10 `RelaxCatalogItemOriginFkToSetNull` (**D95**), which is why the count below is ten rather than nine.

**Documentation reconciliation performed in this phase:** `ARCHITECTURE_DECISIONS.md` gained **D72–D95** (D72–D79 were made in the first Phase 10 session and referenced by code but never written down; D92–D95 record the QA rounds' decisions), `CLAUDE.md` §2 and §23 were corrected and extended, `BusinessRules.md` BR-10 now records that the reopen action it anticipated exists, `StateMachine.md` §2.3 gained the `ChangesRequested → InReview` row, `PermissionMatrix.md` gained rows for reopening a visit and editing a line, and `Architecture.md` §5.2 gained rows for both new endpoints. **Verified on 2026-09-04 at `090aef0`:** backend **1,645 tests** (372 Domain + 441 Application + 390 Infrastructure + 442 Api), frontend **74 tests**, frontend build **389.68 kB**, 0 warnings, migrations **ten**.

**What Phase 10 does *not* deliver, and is the next phase's work:** the customer-facing half of the quote workflow — email delivery, the magic-token quote page, Accept/Decline and status propagation. The API behind it exists and is tested (`GET`/`POST /api/v1/public/angebote/{token}`, verified working in QA); what is missing is the customer-facing page and a configured mailbox (SRS **OQ-3b**, deployment configuration — `Email:Enabled` is `false` in development, so `LoggingNoOpEmailSender` is used and nothing is actually delivered).
...
**Phase 9 — Email Service Integration — ✅ complete and merged** (PR #16, merge commit `6cd8856`). All six slices are done and the completion gate is closed: MailKit SMTP transport with the six frozen German templates and refuse-by-default production behaviour (Slice 1); the single delivery-failure boundary (Slice 2); the Infrastructure-owned `NotificationDeliveries` table and **migration #9** (Slice 3); `GET /api/v1/notification-deliveries` (Slice 4); and manual Admin retry via `POST /api/v1/notification-deliveries/{id}/retry` (Slice 5). **`PHASE9_PROGRESS.md` records the eight approved decisions (F1–F8), the per-slice decisions S1-* through S5-10, and the frozen email copy** — all written before the code they govern, in the same discipline as Phase 8's thirteen. Published and merged with explicit permission, per `CLAUDE.md` §19.
...
- **`claude/customer-workflow-phase-rvh4rj` is the current branch** (Phase 11, the customer-facing workflow), off `main` at `a5124ca`. Slice 1 of eight is implemented; see `PHASE11_PROGRESS.md` §3 for the slice list and §4.5 for what still needs verifying.
- **Next step:** open the Slice 1 PR, confirm CI green (including `has-pending-model-changes`), then Slice 2 — the customer page skeleton. **No slice starts before the previous one is green.** Per `CLAUDE.md` §19, no direct commits to `main`, no force-push ever, and no push or PR without explicit permission.
...
- `dotnet ef migrations has-pending-model-changes` → no pending changes (**ten** migrations: `InitialCreate`, `AddAuditLog`, `AddNumberSequence`, `AddIdentity`, `AddRefreshTokens`, `AddTokenLinks`, `AddCustomersAndProjects`, `AddInvoicesAndPayments`, `AddNotificationDeliveries`, `RelaxCatalogItemOriginFkToSetNull`). **Phase 9 Slice 5 added the `Sending` status with no migration** — `NotificationDeliveries.Status` is string-converted, so a new enum member costs nothing. **Phase 10's tenth migration is a referential-action change only** (`Restrict` → `SetNull` on `CatalogItems.CreatedFromAngebotItemId`, **D95**); no table, column or data changed.
  - `RenoTrack.Domain.Tests`: **372 tests.**
  - `RenoTrack.Application.Tests`: **441 tests.**
  - `RenoTrack.Infrastructure.Tests`: **390 tests** (real SQL Server LocalDB integration tests; `LoggingNoOpEmailSenderTests`/`DependencyInjectionTests`/`LocalDiskFileStorageTests`/`TokenLinkServiceTests` open no database connection — `LocalDiskFileStorageTests` uses real disk I/O in a temporary root).
  - `RenoTrack.Api.Tests`: **442 tests** (real `WebApplicationFactory<Program>` against real LocalDB, schema via `MigrateAsync`, D58 — including Phase 6's public token-link surface and rate-limiting coverage; `PublicRateLimitPartitionTests` is the one class here that runs against plain `HttpContext` objects rather than the host, deliberately, because `TestServer` supplies no `RemoteIpAddress`).
...
- **`IEmailSender` → `LoggingNoOpEmailSender` — ✅ done (Slice 13).** `src/RenoTrack.Infrastructure/Email/LoggingNoOpEmailSender.cs` — never throws (unlike `PlaceholderFileStorage`; the interface's own doc comment explicitly sanctions a no-op/logging placeholder here so Phase 2's handlers run end-to-end without SMTP), but logs a `Warning` on every call so it's never silent. **Superseded as the default by Phase 9's `SmtpEmailSender` (MailKit, D68), and deliberately retained**: it is still what resolves whenever `Email:Enabled` is false, which is every test host and every non-production one.
...
10. **`LocalDiskFileStorage` — ✅ built in Phase 4 Slice 8** (D42's deferral resolved). **Real `IEmailSender` — ✅ built in Phase 9 Slice 1** as `SmtpEmailSender` over MailKit (D68); OQ-3a is resolved and OQ-3b is deployment configuration. `LoggingNoOpEmailSender` is retained deliberately as the `Email:Enabled=false` implementation, not as a placeholder.
...
**Phase 9 — Email Service Integration. ✅ complete; all six slices done and the completion gate closed.** Design approved 2026-08-09, implemented on `feature/phase-9-email-integration` (six commits), merged as PR #16. The delivered scope is the *complete* delivery workflow, not SMTP integration alone: MailKit transport and configuration with the six frozen German templates (Slice 1), the single delivery-failure boundary (Slice 2), `NotificationDeliveries` + migration #9 (Slice 3), `GET /api/v1/notification-deliveries` (Slice 4), and manual Admin retry (Slice 5). **OQ-3a is resolved** (SMTP + MailKit, D68); **OQ-3b is deployment configuration** by decision — no company mailbox, address, host or credential is compiled in and none has a default. **Published and merged.** Decisions and per-slice record: **`PHASE9_PROGRESS.md`**; permanent record: **D68–D71**.

**One deliberate, documented constraint carried out of Phase 9:** the two customer-facing emails link to `https://<public-site>/angebot/{token}` and `/invoice/{token}`, and **the Website pages those URLs point at do not exist yet — Phase 13 builds them** (`PHASE9_PROGRESS.md` D4.2). Production customer-facing use of those two notifications therefore depends on Phase 13. This is a sequencing constraint that was agreed in advance, not a gap.

...
**Phase 10 — Dashboard (Angular) — ✅ complete on `feature/phase-10-dashboard`, uncommitted, unpushed, no PR.** See `PHASE10_PROGRESS.md`. It delivers the roadmap's Phase 10 **and** Phases 11–12 (Angebot builder, Catalog picker, Angebot review) **and** the Project/Invoice screens, because a Dashboard that stopped at the Lead pipeline could not reach the commercial workflow the system exists for. Every commercial workflow is real and integrated: Lead → Inspection → Angebot (build, submit, review, approve, send) → customer decision → Project → Invoice (create, send, mark paid, void) → Project completion with FR-8.6's override.
...
**Documentation reconciliation performed in this phase:** `ARCHITECTURE_DECISIONS.md` gained **D72–D91** (D72–D79 were made in the first Phase 10 session and referenced by code but never written down), `CLAUDE.md` §23 was extended, and five `PermissionMatrix.md` rows plus three `Architecture.md` §5.2 sections now name the routes that serve them. Verified figures: backend **1,602 tests** (354 Domain + 438 Application + 386 Infrastructure + 424 Api), frontend **45 tests**, frontend build **386.11 kB**, 0 warnings, migrations **nine**.
...
**Phase 9 — Email Service Integration — ✅ complete on `feature/phase-9-email-integration`, not yet published.** All six slices are done and the completion gate is closed: MailKit SMTP transport with the six frozen German templates and refuse-by-default production behaviour (Slice 1); the single delivery-failure boundary (Slice 2); the Infrastructure-owned `NotificationDeliveries` table and **migration #9** (Slice 3); `GET /api/v1/notification-deliveries` (Slice 4); and manual Admin retry via `POST /api/v1/notification-deliveries/{id}/retry` (Slice 5). **`PHASE9_PROGRESS.md` records the eight approved decisions (F1–F8), the per-slice decisions S1-* through S5-10, and the frozen email copy** — all written before the code they govern, in the same discipline as Phase 8's thirteen. Pushing, opening a PR and merging still require explicit permission (`CLAUDE.md` §19).
...
- **`feature/phase-10-dashboard` is the current branch** (Phase 10, above). **Local only, uncommitted, never pushed, no PR.**
- **`feature/phase-9-email-integration`**, branched off `main` at `d7ae2f0`, carrying the six Phase 9 commits. **It is local only — never pushed, no upstream configured, and no PR exists.**
...
- **Next step:** publish Phase 9 (push `feature/phase-9-email-integration`, open a PR, merge) — **all three require explicit permission**, and none has been given. After that, Phase 10 per `PROJECT_ROADMAP.md`. Per `CLAUDE.md` §19, no direct commits to `main`, no force-push ever, and no push or PR without explicit permission.

## 3. Build & Test Status (verify this yourself before trusting it — it may be stale)

As of the Phase 9 completion gate, on `feature/phase-9-email-integration`:
- `dotnet build RenoTrack.slnx` → **0 Warnings, 0 Errors** in **both Debug and Release**.
- `dotnet test RenoTrack.slnx` → **1,485 tests passing, 0 failing, 0 skipped.** (The Phase 8 merge baseline was **1,324**; Phase 9 Slices 1–3 together +106, Slice 4 +24, Slice 5 +31, Slice 6 +0 — Slice 6 added no code and no tests.)
...
- `dotnet ef migrations has-pending-model-changes` → no pending changes (**nine** migrations: `InitialCreate`, `AddAuditLog`, `AddNumberSequence`, `AddIdentity`, `AddRefreshTokens`, `AddTokenLinks`, `AddCustomersAndProjects`, `AddInvoicesAndPayments`, `AddNotificationDeliveries`). **Phase 9 Slice 5 added the `Sending` status with no migration** — `NotificationDeliveries.Status` is string-converted, so a new enum member costs nothing.
  - `RenoTrack.Domain.Tests`: **332 tests.**
  - `RenoTrack.Application.Tests`: **422 tests.**
  - `RenoTrack.Infrastructure.Tests`: **363 tests** (real SQL Server LocalDB integration tests; `LoggingNoOpEmailSenderTests`/`DependencyInjectionTests`/`LocalDiskFileStorageTests`/`TokenLinkServiceTests` open no database connection — `LocalDiskFileStorageTests` uses real disk I/O in a temporary root).
  - `RenoTrack.Api.Tests`: **366 tests** (real `WebApplicationFactory<Program>` against real LocalDB, schema via `MigrateAsync`, D58 — including Phase 6's public token-link surface and rate-limiting coverage; `PublicRateLimitPartitionTests` is the one class here that runs against plain `HttpContext` objects rather than the host, deliberately, because `TestServer` supplies no `RemoteIpAddress`).
...
**Phase 9 — Email Service Integration. ✅ complete; all six slices done and the completion gate closed.** Design approved 2026-08-09, implemented on `feature/phase-9-email-integration` (six commits, local only). The delivered scope is the *complete* delivery workflow, not SMTP integration alone: MailKit transport and configuration with the six frozen German templates (Slice 1), the single delivery-failure boundary (Slice 2), `NotificationDeliveries` + migration #9 (Slice 3), `GET /api/v1/notification-deliveries` (Slice 4), and manual Admin retry (Slice 5). **OQ-3a is resolved** (SMTP + MailKit, D68); **OQ-3b is deployment configuration** by decision — no company mailbox, address, host or credential is compiled in and none has a default. **The next step is publication, which requires explicit permission.** Decisions and per-slice record: **`PHASE9_PROGRESS.md`**; permanent record: **D68–D71**.

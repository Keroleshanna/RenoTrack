...
| Projects | `POST /api/v1/projects/{id}/complete` | Admin (`PermissionMatrix.md` §5 "Mark Project Completed (incl. override)"). **Built in Phase 8 Slice 6**; this row was missing even though §5 granted the action, StateMachine §4.3 defined the transition and Sequence §10 named the route. Body `{ forceOverride, reason }` is **optional** — omitting it means no override; the Admin comes from the token and the Project from the route (D61). **409** when the Project is not `Active` — `Project.Complete()` refuses that on its own, it is checked **first**, and **no override reaches it**, so a non-`Active` Project always reports its own state and its Invoices are never read — and separately when the Project's Invoices block it (none at all, or one or more `Draft`/`Sent`/`Overdue`) and no override was given. **400** for `forceOverride` without a reason (FR-8.6), a reason without `forceOverride` (rejected, never silently dropped), and `forceOverride` when nothing is blocking. Returns 200 with `ProjectDto`. Audited as `ProjectCompleted` against the Project, the reason in `details` on the override path — **the reason's only storage**, since `ERD.md` defines no column for it |

...
| Leads | `POST /api/v1/leads/manual` | Admin only (`PermissionMatrix.md` §1 "Create Lead manually", SRS FR-2.1). **Built in Phase 10.** A separate route from the anonymous contact form above, because one route cannot be both anonymous and Admin-only — `Sequence Diagram.md` §2 showed them sharing one and was corrected (**D86**). Body is `{ name, phone, email, source, address, notes }`; `source` is typed so that only `Phone`/`Email` are expressible, which is what keeps FR-9.2's "new website Lead" notification unreachable from this path. The creating Admin comes from the token (D61) and attributes the audit entry. Returns 201 with a `Location` |
| Leads | `GET /api/v1/leads`, `GET /api/v1/leads/{id}` | Dashboard, Admin/Inspector (scoped) |
| Leads | `PUT /api/v1/leads/{id}` | Corrects contact details (`PermissionMatrix.md` §1 — Admin `F`, Inspector `S`, enforced by `IOwnershipValidator` as 403). **Built in Phase 10.** Body is `{ name, phone, email, address }` — all four replaced, so an omitted address clears it. **`notes` is deliberately absent** (**D87**): §1 grants "contact details" and FR-2.1 lists notes separately. No status guard — clerical correction is not a lifecycle transition, so BR-7 is untouched and the Lead's status never moves. Audited as `LeadContactDetailsUpdated` |
| Leads | `PUT /api/v1/leads/{id}/inspector` | Assigns or reassigns the responsible Inspector (`PermissionMatrix.md` §1 — Admin `F`), independently of scheduling a visit, which assigns as a BR-13 side effect. **Built in Phase 10.** Body is `{ inspectorId }` — "who is acted upon", so a legitimate body field under D61 as corrected in Phase 4 Slice 7. **404** when the id names no active Inspector, which covers missing, deactivated and wrong-role alike (D62). Never changes `Lead.Status`. Audited as `LeadInspectorAssigned` with the target id in `details` |
...
| Inspections | `POST /api/v1/inspections/{id}/reopen` | Reopens a completed visit so its record can be corrected (**BR-10**, **D92**). **Built in Phase 10 QA.** Assigned Inspector only — §2 gives Admin `—` on photos, notes and completion, so the action that re-enables those edits belongs to whoever was on site; ownership is enforced in the handler. No request body. **This is the escape hatch BR-10 itself named, not a relaxation of it:** photo upload, notes and reassignment still refuse while the visit is complete. The Lead deliberately stays at `InspectionDone` — the visit happened, and any Angebot built on it stays valid. **409** if the visit was never completed. Audited as `InspectionReopened` against the **Lead**, which is what preserves the completion once `CompletedAt` is cleared. Re-completing afterwards is handled by **D93**: the Lead-level transition fires once, not once per completion |
| Inspections | `PUT /api/v1/inspections/{id}/inspector` | Moves a scheduled visit to a different Inspector (`PermissionMatrix.md` §2 — Admin `F`). **Built in Phase 10.** Body is `{ inspectorId }` (D61's "who is acted upon"). **Re-applies BR-13** — the Lead's assigned Inspector moves with the visit in the same commit, or the outgoing Inspector would keep it in their server-side-filtered pipeline while the incoming one could not see it. **409 once the visit is completed**: BR-10 makes it immutable evidence of who attended, so `Inspection.Reassign` refuses exactly as `AddPhoto` does (**D89**). **404** when the target names no active Inspector (D62). Audited as `InspectionReassigned` against the **Lead**, following `InspectionScheduled` |
...
| Angebote | `PUT /api/v1/angebote/{id}/items/{itemId}` | Corrects an existing line while the Angebot is editable (`PermissionMatrix.md` §3, owning Inspector). **Built in Phase 10 QA**, on an explicit Product-Owner requirement: a typo could previously only be fixed by deleting and recreating the line, which also discards any Catalog entry contributed from it (FR-4.10). Body carries every editable value and replaces all of them; **no `sectionId`** (the handler finds the owning section, so a caller cannot quietly move a line while claiming to edit it) and **no `catalogItemId`** (editing has one mode — re-pointing a line at a different Catalog entry is a different line). Returns the refreshed totals and VAT breakdown, not the line, because a quantity or price change moves the document's money (D81). **409** once the Angebot leaves `Draft`/`ChangesRequested`; **400** for a blank unit or a non-positive quantity. A Catalog-sourced line is editable and keeps its provenance link — BR-8 makes the copy independent at creation |
| Angebote | `GET /api/v1/angebote/{id}` | The full tree — header, sections, items, VAT breakdown. Admin `F`, Inspector `S`. **Added in Phase 5**: internal review is impossible without a read, and this table previously listed only writes. **Phase 10 QA added `savedToCatalog` per line** — whether a Catalog entry was created *from* that line (FR-4.10), which is the opposite direction to `catalogItemId` and not interchangeable with it. The screen had been using `catalogItemId` to decide whether to offer "save as Catalog item", so the action stayed on offer after it had already been used |
...
| Angebote | `POST /api/v1/angebote/{id}/resend` | Admin | Re-issues the customer's token link (FR-6.1a, **D99**, Phase 11 Slice 6). 200 with `AngebotDto` and **no token** — the credential reaches the customer only by email, as with `/send`. **409** when the Angebot is not `Sent`, when the existing link has already carried a decision, and when a concurrent re-issue won the race. The previous link is expired in the same transaction that creates the new one, so at most one credential is ever usable |
...
| Angebot decision (public) | `POST /api/v1/public/angebote/{token}/decision` | Lead approves/rejects. **Built in Phase 6.** Body is `{ decision, reason? }` — the optional FR-6.3 rejection reason was added in Phase 11 Slice 5 (**D98**), is accepted **only** with a rejection (an approval carrying one is a 400, per K-4/D67's rule), is capped at 1000 characters, and is **never echoed to the customer or placed in an email** — staff read it on the Angebot detail page. Consumes the link, records the Angebot decision and moves the Lead to `Won`/`Lost` in one transaction (StateMachine §5). 409 if the link has already decided |
...
| Projects | `POST /api/v1/projects/{id}/hold`, `POST /api/v1/projects/{id}/resume` | Admin (`PermissionMatrix.md` §5 "Put Project On Hold / Resume", StateMachine.md §4.3). **Built in Phase 10**; the Domain methods existed from Phase 7 and were unreachable. **Two named endpoints rather than one status setter** — BR-7's reasoning, and the same reason `PATCH /leads/{id}/status` was removed from this table (**D90**). **No request body and no reason**: §4.3 mentions an optional reason but `ERD.md` defines no column for one, and the only home would be a best-effort audit row (D50). **409** from any state other than `Active`/`OnHold` respectively; the aggregate refuses and the handler does not re-check. Pausing does **not** block invoicing — StateMachine §5 permits an Invoice against `Active` *or* `OnHold`. A paused Project cannot be completed (§4.2 draws no such edge). Audited as `ProjectPutOnHold`/`ProjectResumed` |
...
| Notification deliveries | `GET /api/v1/notification-deliveries` | Admin only (`PermissionMatrix.md` §9 — Admin `F`, Inspector `—`), so no ownership check. **Built in Phase 9 Slice 4.** The operational view of email delivery, required because a committed business operation stays successful when its notification fails (D69) and two of the six senders are anonymous endpoints where no Admin is present to be told. Takes `?status=` plus `?page=`/`?pageSize=` per §5.1, ordered `CreatedAt DESC, Id DESC`. **Omitting `status` returns every status including `Sent`** — §9 says "failed/pending", but hiding successes would make "did my retry work?" unanswerable. Returns the twelve persisted columns flat: `EntityType`/`EntityId` are **not** resolved to a title or link (polymorphic, no FK), a null `Recipient` stays JSON `null`, and `FailureMessage` is the sanitized text as stored. **Deliberately outside the CQRS pipeline** — the record and its enums are Infrastructure-owned (D69), so the controller consumes `INotificationDeliveryQueries` directly, on the same test D60 applies to `AuthController`. No `entityType`/`entityId` filter ships yet |
| Notification deliveries | `POST /api/v1/notification-deliveries/{id}/retry` | Admin only (`PermissionMatrix.md` §9), no ownership check. **Built in Phase 9 Slice 5.** No request body — the delivery comes from the route and the Admin from the token (D61). Re-sends **only** the notification, rebuilt from currently persisted business data; it never re-executes the business operation and **never mints a token** (D70). Claims the row with one atomic conditional `UPDATE` (`Failed`/`Pending`/`Sending` → `Sending`, `AttemptCount + 1`), so two Admins double-clicking produce one send and one 409. **`Sending` is retryable on purpose**: with no lease or sweeper anywhere, a crashed attempt is recoverable only by hand. **Every refusal is 409** — already `Sent`, a lost claim race, an expired or already-used `TokenLink`, a `Void`/`Paid` Invoice, and `Email:Enabled=false`; 404 for an unknown id. A *delivery* failure is not a refusal: it returns **200** with the row recording `Failed`. Updates the existing row and never inserts a second one |
...
- Rate limiting / basic abuse protection on public endpoints (`/api/v1/public/...` and the contact form) to prevent scraping or brute-forcing token guesses. **Partially implemented as of Phase 6, and the split is deliberate:**
  ...
  - **Trust boundary — the code half exists as of Phase 11 (`ARCHITECTURE_DECISIONS.md` D97); the deployment half is configuration.** The limiter still partitions on the connection's `RemoteIpAddress`, and `X-Forwarded-For` is still ignored unless `TrustedForwarders:KnownProxies`/`KnownNetworks` explicitly names the forwarder — **empty is the default and means trust nothing**, so an unconfigured deployment behaves exactly as it did before D97 rather than believing a header any caller can set. A malformed entry fails startup naming the key, because a trust list that silently shrinks yields a limiter that looks configured and is not. **Why it became urgent:** `RenoTrack.Website` now renders the customer's Angebot page server-side and calls the API on the customer's behalf, so without a named forwarder every customer would share one 30-per-minute bucket. The Website forwards the *connection's* address, never a header the customer supplied.

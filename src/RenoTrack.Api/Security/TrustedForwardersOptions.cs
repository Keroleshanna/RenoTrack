...
/// <summary>
/// The explicit allowlist of proxies whose <c>X-Forwarded-*</c> headers the API will believe,
/// bound from the <c>TrustedForwarders</c> configuration section (D97).
/// </summary>
/// <remarks>
/// <para>
/// <b>This is the trust boundary D65 declined to invent.</b> The public rate limiter partitions on
/// <c>Connection.RemoteIpAddress</c> and has always refused to read <c>X-Forwarded-For</c>, because
/// believing a forwarded header with no known proxy trust boundary lets any caller mint a fresh
/// partition per request and defeat the limiter entirely. D97 supplies the missing half: with a
/// forwarder named here, and only then, the limiter sees the real customer again instead of
/// collapsing every customer behind the Website into one bucket.
/// </para>
/// <para>
...
/// </para>
/// <para>
/// <b>Deliberately duplicated between the API and the Website</b> rather than shared. The Website
...
public sealed class TrustedForwardersOptions
{
    ...
    public ForwardedHeadersOptions Build()
    {
        var options = new ForwardedHeadersOptions
        {
            ...
            // One hop by default: the API expects exactly one trusted forwarder in front of it — the Website, or a
        // reverse proxy.
